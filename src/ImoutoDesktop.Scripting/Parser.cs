...
internal class Parser
{
    ...
    private readonly List<Define> _globalDefines = new();
    ...
    private string ExecPreProcessor(string line, List<Define> define)
    {
        ...
        length = _globalDefines.Count;
        for (var i = 0; i < length; i++)
        {
            line = _globalDefines[i].Before.Replace(line, _globalDefines[i].After);
        }
        return line;
    }
        ...
        else
        {
            ...
            if (arguments != null)
            {
                ...
                for (var i = 0; i < length; i++)
                {
                    ...
                    {
                        expressions[i] = new ValueExpression(Value.Empty);
                    }
                }
    ...
    private IExpression MakeExpression11(string[] tokens, ref int index)
    {
        ...
        else
        {
            ...
            else if (bool.TryParse(tokens[index], out var bValue))
            {
                // Boolean
                expression = new ValueExpression(new Value(bValue));
            }
            else if (int.TryParse(tokens[index], out var iValue))
            {
                // Int32
                expression = new ValueExpression(new Value(iValue));
            }
            else if (double.TryParse(tokens[index], out var dValue))
            {
                // Double
                expression = new ValueExpression(new Value(dValue));
            }
